...
// -------------------------------------------------------
// Fonction mutualisée : parser INI manuel permissif
// Utilisée par config.php et infosvx.php
// -------------------------------------------------------
function parse_svxlink_config($filePath) {
    $config = [];
    $currentSection = '';
    ...
    foreach ($lines as $line) {
        ...
        // Ignorer les commentaires (;) et les lignes sans contenu utile
        if ($line === '' || strpos($line, ';') === 0 || strpos($line, '#') === 0) {
            continue;
        }

        // Détecter les sections [NomSection]
        if (strpos($line, '[') === 0 && strpos($line, ']') !== false) {
            $currentSection = trim(substr($line, 1, strpos($line, ']') - 1));
        ...
        // Lire les paires key = value uniquement si on est dans une section
        if ($currentSection && strpos($line, '=') !== false) {
            list($key, $value) = explode('=', $line, 2);
            $config[$currentSection][trim($key)] = trim($value, ' "');
        }
        // Les lignes sans '=' sont simplement ignorées (pas d'erreur)
    }
