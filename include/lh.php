...
<span style="font-weight: bold;font-size:14px;">::&nbsp;Activity SVXReflector M.A.R.R.I For <a style="color: #b30000; font-size: 11pt"><?php echo $CALLSIGN; ?></a>&nbsp;::</span>
...
for ($i = 0;  ($i <= 24); $i++) { //Last 25 calls
	if (isset($lastHeard[$i])) {
		...
		if ( $listElem[1] ) {
                        // strftime() remplacé par date() - compatible PHP 8.1+
                        $timestamp  = strtotime($listElem[0]);
                        $local_time = ($timestamp !== false) ? date("d M Y H:i:s", $timestamp) : $listElem[0];
